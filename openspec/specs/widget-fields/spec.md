# widget-fields Specification

## Purpose

定义 Peregrine 前端共享字段组件规范：所有共享字段组件（slider/number/text/color/toggle/select/image_path）遵循统一两行布局（label 一行 + 控件一行），位于 `src/components/fields/` 统一目录，基于 shadcn primitive 构建，供参数面板、图层样式、图层编辑等多处复用。
