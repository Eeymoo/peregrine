...
- （2026-08-03 二次修订）**单/多图层切换入口全部保留，切换功能可用**：预览工具栏「切换到多图层」按钮、异常兜底按钮、不兼容 profile 的「切换到图层编辑器」按钮均**不再**以 `MATERIAL_RUNTIME_ENABLED` 门控。软关闭只作用于渲染链路（overlay 与预览走旧版 `Crosshair` 路径），不锁死 UI 模式切换——不兼容 profile 的唯一完整编辑途径是图层编辑器，入口关死会让多图层配置无法编辑甚至被单图层保存改坏。
- （2026-08-03 修订）`layersMode` 初始值**从 localStorage 持久化值恢复**（原决策为「初始值强制 `false`」，经实测推翻——模式恢复不做软关闭特判，与其他时期行为一致，即「怎么关闭的就怎么展开」）。
- 已知取舍：软关闭期间在图层编辑器中的修改**不影响 overlay 渲染与预览**（均走 crosshair 路径），图层数据正常保存，待运行时重新启用后生效；不兼容提示区保留「多图层功能已暂时停用」文案以说明此状态。
- `LayersEditor.tsx`、`LayerPanel.tsx`、`LayerEditors.tsx` 组件文件保留不删。
- i18n 文案键保留。
