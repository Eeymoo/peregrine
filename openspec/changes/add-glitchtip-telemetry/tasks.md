...
- [ ] 6.5.2 release 构建（`panic = "abort"`）下 PGR-2101 / PGR-4101 错误事件正常上报（code / 函数名 / 文件 / 行号齐全）—— 需 Windows 环境
...
> **延后到本 change 代码 + 验收稳定后统一编写**（避免边改边写、写完又改；文档仍归本 change，时序在后）。Code 常量集中定义于遥测模块（`telemetry.rs::report_code` / `src/lib/telemetry.ts::REPORT_CODES`）即满足治理要求；`REPORT_CODES.md`、docs 文档站链接等开发者向文档在代码稳定后一次性产出。
...
- [ ] ~9.3 更新 `AGENTS.md`：新增遥测模块说明、DSN 注入方式、safe_try! 使用约定（仅关键路径）、隐私开关语义、上报 CODE 登记治理约定、编译期禁用方式~ **【延后：代码 + 验收稳定后统一编写】**
- [ ] ~9.4 更新 docs 文档站（用户指南：隐私说明与开关；开发指南：DSN 环境变量、编译期禁用与本地调试）~ **【延后：代码 + 验收稳定后统一编写】**
...
- [ ] 10.15 所有上报事件（启动/崩溃/错误）均携带 `code` tag，GlitchTip 中可按 code 筛出同一来源事件；Code 常量集中定义于遥测模块（Rust `report_code` / 前端 `REPORT_CODES`），无散落硬编码（`REPORT_CODES.md` 开发者向文档延后到代码 + 验收稳定后统一编写，本 change 验收阶段不验证文档）
