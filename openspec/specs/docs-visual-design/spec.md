# docs-visual-design Specification

## Purpose

定义 Peregrine 文档站（`peregrine.aukcraft.org`）的视觉设计系统与落地页体验要求：Tailwind CSS v4 设计系统集成与品牌 token 单一事实源、首页落地页构成（Hero / 特性 / 上手步骤 / 下载区）、真实截图资产管线、文档页排版质量与明暗双主题一致性、动效克制与可访问性。
