...
- **`ci.yml`**: triggered on push to main/master or on pull requests. The `build` job runs in a three-platform matrix of **Windows (x86_64-msvc) / macOS (aarch64 + x86_64 cross-compile) / Linux (x86_64-gnu)**, executing `cargo test -p peregrine_config --locked` + `npm ci && npm run build && cargo build --manifest-path src-tauri/Cargo.toml --bins --features tauri/custom-protocol --release --target <target>` (Tauri entry). The Linux job first installs GUI dependencies (xcb/x11/wayland/gtk/WebKit, etc.). CI does not package NSIS, avoiding Windows build failures due to missing signing keys. The `lint` job only runs on Linux: `cargo fmt --all -- --check` and `cargo clippy -p peregrine_config -- -D warnings`.
