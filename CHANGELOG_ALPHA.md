...
- **SVG 渲染后端**：覆盖层新增可选 SVG 渲染后端（resvg + tiny-skia），在「设置 → 覆盖层 → 渲染后端」中切换。SVG 模式抗锯齿质量更高；CPU 模式（默认）零额外依赖、更轻量。两套方案完全并行，SVG 光栅化失败时自动回退到 CPU。 @Eeymoo
- **滚动条样式优化**：自定义滚动条样式，默认透明、悬停淡入，6px 宽圆角，与应用风格统一。 @Eeymoo
...
- **预览比例修复**：预览区以真实分辨率构建准心形状再等比缩放，预览所见即覆盖层所得，不再因 canvas 尺寸导致比例失真。
- **静态准心不再持续重绘**：引入脏标记机制，静止不动的准心不再每帧重绘，显著降低覆盖层 CPU 占用。
- 文档新增「更新日志」页面（中英文），侧边栏加入入口。
- 文档「功能特性」页新增全屏/窗口模式、拖拽实时显示、GPU 开关、设置项说明。
- `cargo fmt` 格式化全部 Rust 代码，修复 CI lint 失败。

### 修复

- **文档部署失败**：VitePress 构建时继承根目录 `postcss.config.js` 导致 CI 找不到 tailwindcss 模块，显式置空 PostCSS 配置修复。
