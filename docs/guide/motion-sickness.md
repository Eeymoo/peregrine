...
这就是 Peregrine 的核心原理——在屏幕上叠加一个**固定的视觉锚点**（十字准星、圆环、边缘标记等）。当游戏画面剧烈晃动时，这个锚点始终纹丝不动，眼睛会把它当作「地面」或「地平线」，从而减轻感官冲突。
...
- 如果中心十字准星影响瞄准，可以试试 `EdgeRect`（边缘矩形）或 `BorderFrame`（边框），把锚点放在边缘。
