...
- **新增 `safe_try!` 宏**（错误上报语义，非 panic 捕获）：包装返回 Result/Option 的关键路径，按故障域分号段覆盖：后端通用 IO（配置/贴图，PGR-2xxx）、遮盖层（overlay 渲染 / Win32 窗口设置与跟随，PGR-4xxx）、Tauri command 操作域（配置/图层/覆盖层/更新/遥测操作，PGR-5xxx）；失败时自动携带函数名 / 文件 / 行号上报，不依赖 `catch_unwind`（release 的 abort 模式下失效）
- **匿名启动统计**：启动后上报一条 `Info` 级事件（不进 issue 列表），携带 `install_id` / `version` / `os` / `arch` tag，用于按安装去重估算用户量。授权弹窗文案使用「使用情况」一词（而非「使用统计」），为未来功能使用分析埋点（`feature_used` / `overlay_created` 等，不在本 change 范围）预留叙事余地
...
- **上报 CODE 体系**：所有上报事件（启动/崩溃/错误）统一携带 `code` tag，按故障域（前端 PGR-3xxx / 后端 PGR-2xxx / 遮盖层 PGR-4xxx）与操作域（PGR-5xxx）分号段，首批 20 个 Code；Code 常量集中于遥测模块（Rust `report_code` / 前端 `REPORT_CODES`），新增上报点必须先登记常量方可合入；启动事件为首条 Code（`PGR-0001` = app_startup）。**`REPORT_CODES.md` 开发者向文档与 docs 文档站遥测章节延后到本 change 代码 + 验收稳定后统一编写**（避免边改边写；文档仍归本 change，时序在后）
