...
- [ ] 6.1 多图层配置 overlay 按图层叠加渲染（验收场景：多图层配置按图层渲染）
- [ ] 6.2 图层编辑保存后 overlay 与预览即时更新（WYSIWYG）
- [ ] 6.3 12 种内置物料逐样式渲染目检（确认文本图元等无渲染退化）
- [ ] 6.4 物料选择器不出现 `builtin.time`；CPU 无动态轮询空转
- [ ] 6.5 纯旧 crosshair 配置渲染外观不变
