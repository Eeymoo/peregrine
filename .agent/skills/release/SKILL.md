...
打 tag 并推送前，必须向用户确认以下事项：

1. **版本号** — 确认版本号符不符合 SemVer（major/minor/patch 是否合理），正式版还是预发布。
2. **Tag 消息** — 展示完整的 tag 提交信息（即 Release body），让用户审阅。
3. **最后机会** — 推送 tag 会触发真实构建发布，等待用户明确回复"确认"或"推送"再执行。
