...
The settings window is opened from the tray menu ("Settings") or the config panel. It contains 7 tabs: **General / Overlay / Material / Hotkeys / Update / About / Dev** (the "Dev" tab appears only after developer mode is unlocked). This page covers the first five; for the full field definitions see the [Configuration Guide](/guide/config/).

:::note[About the screenshots]
The screenshots show the **main app's own fixed light UI**; they do not change with the docs site's light/dark theme.
:::

<Mermaid
  chart={`graph TD
    W["Settings window"] --> G["General<br/>Language / GPU acceleration / Telemetry"]
    W --> O["Overlay<br/>Start behavior / Drag preview / Anti-aliasing / Renderer backend / Quick colors"]
    W --> M["Material<br/>Dynamic material switch / Animation FPS"]
    W --> H["Hotkeys<br/>Action ↔ key bindings"]
    W --> U["Update<br/>Update channel / Check for updates / Mirror"]
    W --> A["About (not covered here)"]
    W --> D["Dev (shown after developer-mode unlock)"]
...
- **Telemetry**: Consent switch for anonymous crash reports + launch statistics (see [Privacy & Telemetry](/guide/privacy/)). Changes take effect after a restart; choosing "restart later" shows a pending-restart badge. The whole item is hidden when the build has no DSN injected (e.g. self-compiled).
...
Layer management and per-layer parameters live in the config window's layer editor — see [Layers](/guide/layers/).
...
This page also provides an "**Open materials folder**" button: it reveals the folder holding user-defined materials (`.rhai` scripts) in your system file manager (created automatically if missing). Dropping in or editing a script hot-reloads automatically — no restart needed.
...
<Mermaid
  chart={`graph LR
    A["Compile-time switch<br/>MATERIAL_DYNAMIC_INPUT_ENABLED"] --> AND{"AND"}
    B["Runtime user switch<br/>dynamic_enabled"] --> AND
    AND -->|"both true"| C["Dynamic pipeline live<br/>input polling / dynamic eval context /<br/>continuous redraw / dynamic materials in the picker"]`}
/>
...
For writing Rhai material scripts see [Material Scripting](/guide/material-scripting/).
...
In-app auto-update is available for the NSIS installer only; portable users should download new versions manually. See [Usage · Update](/guide/usage/).
