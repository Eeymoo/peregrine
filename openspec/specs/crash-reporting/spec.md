...
定义 Peregrine 的崩溃与异常上报体系：Rust panic 同步落盘（兼容 `panic = "abort"`）、关键路径 `safe_try!` 错误宏、React 前端错误出口、上报 CODE 登记规范，以及开发者模式下的测试上报入口。目标是确保任意崩溃/错误信息都能在合规、脱敏、可追溯的前提下上报至 GlitchTip。
