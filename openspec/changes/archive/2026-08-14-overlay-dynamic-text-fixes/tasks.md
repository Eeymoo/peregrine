...
- [ ] 5.4 `cargo check --target x86_64-pc-windows-msvc` 通过【**环境阻塞**：dev 合并引入 updater → ring 依赖需 C 工具链，本机无 MSVC/mingw，无法交叉验证 src-tauri；3 个 lib crate 的 windows-msvc check 已通过，src-tauri 需 Windows 环境验证】

待验证（需要 Windows 运行时环境）：
- [ ] 5.5 时钟物料每秒自动更新，无窗口交互（checklist B4.1）
- [ ] 5.6 纯静态 profile overlay 无空转（任务管理器 CPU 与修复前一致）
- [ ] 5.7 切换静态 ↔ 动态 profile 后重绘调度即时切换
- [ ] 5.8 时间物料「加粗」开关：overlay 与预览同步变粗，目测可读性改善
- [ ] 5.9 物料热重载改 `is_dynamic` 后无需重启 overlay 即时生效
- [ ] 5.10 鼠标跟随 / 键盘响应物料实效复测（依赖 `material-docs-examples` §4 示例物料就位，checklist B4.2/B4.3）
