...
动态物料：以屏幕中心为圆心的圆环，重心偏移由鼠标**加速度**驱动——偏移方向 = 加速度方向（急转弯时水滴朝转弯方向甩出），形变强度 ∝ 加速度幅度。对圆周 ~12 个采样点按偏移方向做径向位移 + 尖角合成，输出闭合 Path（淡填充 + 实描边展示双色）。鼠标静止或匀速移动（加速度 ≈ 0）时退化为正圆——这正是"3D 晕眩多发生在视角急转"场景下锚点应有的行为。schema 声明 `is_dynamic`，预览 ~1s 轮询自动生效。
...
每次 poll_dynamic_context():
  ...
  acc = (vel - prev_vel) / dt
  vel_ema = α·vel + (1-α)·vel_ema          # EMA 平滑，α ≈ 0.4（去轮询抖动）
  acc_ema  = α·acc + (1-α)·acc_ema
