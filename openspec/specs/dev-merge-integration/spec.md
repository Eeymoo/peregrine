# dev-merge-integration Specification

## Purpose

定义 `origin/dev`（v0.1.13 UI 拆分重构）合并进 `feature/four-layer-customization` 分支的验收基线：合并结果 MUST 无冲突残留、版本号统一、dev 新功能与本分支四层架构能力双方零回归，且通过本地全量验证与 PR CI。
