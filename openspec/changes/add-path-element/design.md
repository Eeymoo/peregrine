...
**实机反馈修正记录（v2 → v4，2026-08-15）**：
- **制动门控（v4）**：物料侧判定 `dot(acc, vel) < 0`（加速度与速度反向 = 刹车）→ strength 取 0，不形变。松手急停必然产生与运动方向相反的减速度脉冲，不过滤则「向左移动的最后一下」会朝右甩尖刺（假动作，实机反馈）。加速与转弯（点积 ≥ 0）才是水滴的正信号；速度为 0 时点积为 0 自然放行。
- **滤波尾迹清零（v4，轮询器侧）**：速度死区触发（鼠标静止）时强制加速度输出归零并清空内部 EMA 状态——加速度 EMA 在速度归零后仍残留 ~1/α 帧的「幻影尾巴」（α=0.25 → ~8 帧），是制动尖刺滞后的第二来源。静止结束后从干净的 0 重新起步。
...
**滤波尾迹清零（v4 修正记录）**：EMA 是渐近衰减，速度归零后加速度 EMA 仍残留 ~1/α 帧（α=0.25 → ~8 帧）的非零尾巴——这是"松手后水滴还在抖"的第二来源。轮询器在速度死区触发（鼠标静止）时强制加速度输出归零并清空内部 EMA 状态；物料侧另有制动门控（dot(acc,vel)<0 不形变），双层兜底。
