# telemetry-consent Specification

## Purpose

定义 Peregrine 遥测的隐私授权与开关体系：首次启动授权弹窗、设置页遥测开关（重启生效）、本地 pending 错误存储与报错页面一次性授权上传、上报数据匿名化、编译期禁用开关，以及 DSN 注入与 git 历史隔离规范。目标是确保遥测在「默认允许、可拒绝、可彻底关闭、可编译期禁用」的隐私合规前提下运行。
