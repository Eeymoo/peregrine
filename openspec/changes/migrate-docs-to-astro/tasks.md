...
- [ ] 8.1 `/guide/usage` 与 `/guide/usage.html` 均返回 200
- [ ] 8.2 `/zh-cn/` 返回 200，`/zh-cn` 301 到 `/zh-cn/`
- [ ] 8.3 页面 `Last updated` 显示非空日期
- [ ] 8.4 `/llms.txt` 干净（无幽灵条目、无 QA 文档）
- [ ] 8.5 `/sitemap-index.xml` 可访问
- [ ] 8.6 搜索命中英文与中文关键词
- [ ] 8.7 6 条 `#...` 锚点链接（含 `#动态输入-api`、`#7-配置文件`）逐一可跳转
