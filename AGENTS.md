...
- **Material runtime soft-disabled (change `disable-material-runtime`)**: the layers + Rhai material system is turned off via the compile-time constants `MATERIAL_RUNTIME_ENABLED = false` (`crates/peregrine/src/lib.rs` and `src/lib/feature.ts`). The overlay renderer always uses the legacy `Crosshair` path (`build_shapes`) and ignores `Profile.layers`; dynamic-context polling is still textually present inside the gated `use_new_format` branches but never executed (gated by `crate::MATERIAL_RUNTIME_ENABLED`), and dynamic redraw scheduling is removed from the active path; the settings UI hides the layer editor entries (but the `layersMode` UI mode is persisted in `localStorage` and restored verbatim on startup — "however it was closed, that's how it reopens" — so a stale multi-layer mode from before the soft-disable can still restore into `LayersEditor`; the exit back to single-layer is intentionally NOT gated). All material code/tests are preserved and still compile in CI. To re-enable, flip both constants to `true` (gated sites are greppable via `MATERIAL_RUNTIME_ENABLED`).
