...
### Requirement: 手动测试上报（仅开发者模式）

系统 SHALL 仅在开发者模式（开发构建 / `import.meta.env.DEV`）下于设置页提供「测试上报」按钮，触发一条 `Error` 级测试事件（进入 issue 列表），用于验证上报链路连通性。正式构建 MUST NOT 向普通用户显示该按钮。

#### Scenario: 开发者模式下测试按钮产生测试事件

- **WHEN** 开发者在开发构建的设置页点击「测试上报」按钮且遥测已启用
- **THEN** GlitchTip 收到一条 Error 级测试事件并进入 issue 列表

#### Scenario: 正式构建对普通用户隐藏测试按钮

- **WHEN** 普通用户使用正式构建打开设置页
- **THEN** 设置页不显示「测试上报」按钮
