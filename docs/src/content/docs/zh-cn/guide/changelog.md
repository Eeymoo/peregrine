...
正式版。聚焦**自定义物料的发现与使用体验**：一键打开物料目录、AI 代理可用的官方物料创作 skill、以及新增 `.rhai` 无需重启即可见的热重载兜底修复。

### 新增

- **「打开物料目录」按钮**（设置 → 物料页）：一键在系统文件管理器中打开用户物料目录（`%APPDATA%/Peregrine/materials/`，目录不存在自动创建），省去手动找路径；配套六语文案与设置详解文档说明。 @Eeymoo
- **官方 `material-creation` skill**（仓库 `skills/` 目录，遵循 skills.sh 开放技能生态布局）：面向 AI 编码代理的完整物料创作手册（契约函数 / 图元与 widget 全表 / 动态 API / 锚点设计原则）+ 五个实战配方 + 起步模板；`skills/README.md` 提供 `npx skills add/use` 安装指南与一句话手动安装。安装后对任意代理说「帮我做一个 XX 准心」即可生成并落地 `.rhai`。 @Eeymoo
- **设置详解文档页**（文档站双语）：通用 / 覆盖层 / 物料 / 快捷键逐项说明（含动态物料两层与门图解、FPS 节拍语义表）；配置说明页翻新为 settings + layers 新格式；新增逐 Tab 真实截图。 @Eeymoo

### 修复

- **新增用户物料无需重启即可见**：此前新增 `.rhai` 后物料选择器不刷新、须重启应用。现「添加图层」选择器打开前主动重扫物料目录（后端 `refresh_materials` 命令，通知路径与文件监视器一致），并补齐选择器对 `materials-changed` 热重载事件的监听——文件监视器事件丢失（编辑器原子写 / 杀软拦截）的场景下依然可靠。 @Eeymoo

### 变更

- **物料脚本文档修正**（双语）：热重载描述改为实际的全自动文件监视行为；删除「同名用户物料覆盖内置物料」错误陈述（`user.` / `builtin.` 命名空间独立，经代码验证）；补 `// Name:` 显示名约定与 `parse_svg_path(d)` API 收录。 @Eeymoo

### 构建

- CI：opencode 云代理仅响应人类的 `/oc` 显式召唤，阻断自回复循环；按改动面分流 CI 与 Snapshot 触发（未触及代码的文档变更不再空跑全量构建）。 @Eeymoo

---

## [v0.2.7] — 2026-08-16

尝鲜版。引入 **路径图元**——物料可输出任意矢量几何（贝塞尔曲线）——以及基于它的两个新内置锚点物料、自定义形状的 SVG 路径解析 API，和一批由实机反馈驱动的渲染修复与性能优化。

### 新增

- **路径图元**：物料可输出 `#{type: "path", segments, fill, thickness}`，段命令 `M` / `L` / `Q` / `C` / `Z`（绝对坐标，snake_case 标签）。平滑圆环、花瓣及任何曲线锚点形状均可表达；矩形/圆形继续用专用图元（更便宜）。省略 `stroke_color` / `fill_color` 时继承图层基色 × 图层不透明度，换色热键保持生效。 @Eeymoo
- **内置物料「呼吸环」**（`builtin.teardrop`）：填充环以人类呼吸频率（8 / 14 / 20 次/分三档预设，默认 14 = 成人静息中段）温和涨缩 ±3%。任何时刻都是完美正圆——永不产生不对称形变。通过 10Hz 声明刷新间隔 + 0.5px 输出量化（相同帧整体跳过光栅化），稳态开销保持在静态基线附近。 @Eeymoo
- **内置物料「路径」**（`builtin.path_showcase`）：贴入任意 SVG path `d` 字符串（Figma / Illustrator / Inkscape 导出）即渲染为锚点。任意坐标域自动归一化（包围盒缩放 + 居中）；默认形状为四个 90° 三次贝塞尔段构成的数学精确正圆；非法 `d` 回退内置正圆而非消失。 @Eeymoo
- **物料脚本 `parse_svg_path(d)` host function**：把 SVG path 数据字符串解析为段数组——支持绝对与相对命令（`M/L/Q/C/Z` 及小写，自动换算绝对坐标）、`H/V` 展开、隐式命令重复、逗号/负号分隔符；不支持的 `A/S/T` 返回空数组供脚本侧回退。 @Eeymoo
- **脚本鼠标运动学 API**：`mouse_velocity()`（逻辑像素/秒）与 `mouse_acceleration()`（逻辑像素/秒²），平台层差分采样 + EMA 平滑 + 死区归零（静止时精确为 0，帧稳定）。 @Eeymoo
- **物料脚本文档**：新增路径图元章节（段命令表、字段语义、选型建议）、新动态输入函数、刷新间隔节流章节（`refresh_interval_ms()` + 输出量化），EN 与 zh-cn 双语同步。 @Eeymoo

### 修复

- **SVG 后端继承图层色的路径丢失图层不透明度**：图层色分支输出不透明的 `rgb()` 字符串，默认路径忽略图层透明度滑块；两条颜色路径现统一走同一 paint 闭包（alpha × 图层不透明度）。 @Eeymoo

### 变更

- **呼吸环性能**：早期响应鼠标运动（速度/加速度定格）的设计实机 CPU ~7%（60fps 全屏光栅化）；先移除鼠标跟随（降到 ~3%），再用 10Hz 声明间隔 + 0.5px 量化呼吸半径攻克剩余动态管线开销——可见呼吸以零头成本保留。 @Eeymoo
- **呼吸周期对齐真实呼吸频率**：预设校准为每分钟 8 / 14 / 20 次（原 4.3–10 次，明显慢于自然呼吸）。 @Eeymoo
- **参数面板玩家语言重写**：两个新物料暴露大小 / 宽度 / 速度预设而非工程单位（px/s² 阈值、毫秒周期）；呼吸幅度保持内部参数（3% 调校值——暴露会诱发反效果的「晃动锚点」调参）。 @Eeymoo

## [v0.2.6] — 2026-08-15

正式版本。动态物料链路端到端恢复：内置时钟（`builtin.time`）在覆盖层实时跳动、预览同步刷新，新增「物料」设置页提供运行时开关与动画帧率档位——配合调度节流与字体定向加载，运行中的动态覆盖层 CPU 稳态低于 1%、内存平稳。

### 新增

- **动态物料恢复**：动态链路（输入轮询 → Rhai 求值 → 持续重绘）以「编译期总闸 + 运行时用户开关」双层与门恢复，运行时开关 `settings.material.dynamic_enabled` 默认开启、热生效无需重启。内置时钟物料 `builtin.time` 回到选择器（带「动态」徽章），时间来源改为上下文快照 `time_ms()`，预览与覆盖层保持一致。 @Eeymoo
- **「物料」设置页**：新增设置 Tab，含动态物料开关与动画帧率档位（系统 / 30 / 60 / 120；「系统」跟随主屏刷新率，不可用时回退 60）。帧率是调度上限——纯静态配置不受影响，保持事件驱动；两者均热生效。旧配置经 serde 默认值无感升级（开关开、帧率跟随系统）。 @Eeymoo
- **动态物料实时预览**：激活配置含动态物料且运行时开关开启时，编辑器预览每秒重拉后端，时钟在画布中跳动（与覆盖层相差 ≤1 秒）。 @Eeymoo
- **物料热重载接线**：用户物料目录重载后，覆盖层整体替换新注册表句柄（无需重启），前端经新增 `peregrine:materials-changed` 事件自动刷新物料列表。 @Eeymoo
- **物料级刷新间隔声明**：物料脚本可选导出 `fn refresh_interval_ms()`；调度按「帧率档位与最短声明间隔取较大者」节流唤醒——时钟声明 500ms，唤醒从 60Hz 降到 2Hz，外部事件（配置变更 / 窗口移动）触发的重绘不受限、即时生效。 @Eeymoo

### 修复

- **点击任意处全窗卡死**：`build_shapes_ipc` 在已持有共享配置锁的情况下再次加锁（新增的动态开关读取），图层编辑器预览未传 profile 时主线程同步命令死锁、整个窗口无响应。开关读取已移到取锁之前。 @Eeymoo

### 变更

- **动态覆盖层稳态 CPU / 内存**：求值从每帧两次合并为一次；输出指纹相同的帧直接跳过（不清屏 / 不光栅化 / 不呈现）；usvg 字体库不再每帧重建；tiny-skia Pixmap 复用；字体改为定向 mmap 特定文件（默认 Segoe UI，文本含非 ASCII 时才加载微软雅黑，仅极端情况回退全量扫描）。运行中的时钟覆盖层 CPU 稳态远低于 1%、内存平稳。 @Eeymoo
- **RandomOrb 节拍**随配置帧率档位（原先固定 60FPS）。 @Eeymoo
- 移除图层参数面板的「动态物料（跟随输入）」提示行（无实际指导意义）；选择器「动态」徽章保留。 @Eeymoo
