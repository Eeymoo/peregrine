...
这就是 Peregrine 的核心原理——在屏幕上叠加一个**固定的视觉锚点**（准星、圆环、定位点等）。当游戏画面剧烈晃动时，这个锚点始终纹丝不动，眼睛会把它当作「地面」或「地平线」，从而减轻感官冲突。

这一方法有学术证据支持。韩国 Seok 等人在 *JMIR Serious Games* 发表的研究发现：在 FPS 游戏中开启准心（视觉引导）能**显著降低** VR 眩晕症状，尤其是在使用手柄操作时；准心尺寸为画面比例的 30% 左右、置于头部追踪方向时效果最好[[4]](https://games.jmir.org/2021/3/e18020/)。

Peregrine 将这一原理做成了通用工具：不止准星，你可以选择贴边矩形、中心环、定位球、边框、箭头甚至自定义图片，找到最适合你的锚点形态。
...
- 优先尝试 `Cross`（十字准星）或 `Ring`（中心环），对多数人效果最好。
- 锚点不要太大、不要太亮，半透明即可，避免遮挡游戏画面。
- 如果中心准星影响瞄准，可以试试 `EdgeRect`（贴边矩形）或 `BorderFrame`（边框），把锚点放在边缘。
