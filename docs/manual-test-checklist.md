...
> ✅ **2026-08-14 动态链路已恢复**（change `restore-dynamic-material`）：`MATERIAL_DYNAMIC_INPUT_ENABLED` 翻回 `true`，并有运行时开关（设置 → 物料 → 动态物料，默认开）与帧率档位（系统 / 30 / 60 / 120）。时钟物料 `builtin.time` 重新出现在物料选择器，时间来源为上下文快照。以下条目自该 change 起恢复适用。

- [ ] **B4.1** 添加 `builtin.time`（时钟物料）图层 → overlay 每秒更新（30/60/120 三档 + 系统档分别验证节拍）
- [ ] **B4.2** 添加鼠标跟随物料（examples 目录，需手动拷入用户物料目录）→ 移动鼠标，overlay 延迟 < 50ms
- [ ] **B4.3** 添加键盘响应物料 → 按键即时响应
- [ ] **B4.4** 前端预览动态物料 → 预览画布每秒自动重拉（时钟跳动，与 overlay 显示一致 ±1s）
- [ ] **B4.5** 运行时开关热切换：设置 → 物料 → 关闭「动态物料」→ overlay 即时冻结（无需重启），重新开启即时恢复
- [ ] **B4.6** 纯静态 profile（无动态图层）→ overlay 保持事件驱动，任务管理器 CPU 与软关闭期间一致（FPS 设置不引发空转）
- [ ] **B4.7** 物料目录热重载：修改用户物料 `is_dynamic` 或新增 user 物料 → overlay 与前端物料列表均无重启即感知
