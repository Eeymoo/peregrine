...
Double-click the extracted `peregrine.exe`. The application starts in **settings mode** by default.
...
1. **Select style**: In the settings panel, find **Type** and choose styles such as `Crosshair`, `Center Ring`, or `Edge Rectangle`.
...
2. In the settings panel, switch **Type** to `Custom Image`.
