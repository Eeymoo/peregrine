# release-docs-sync Specification

## Purpose

定义 Peregrine 发版流程与文档站 changelog 之间的强制同步规范：稳定版发版必须遵循「先合 changelog、后打 tag」时序，并通过 CI 闸门强制校验 changelog 最新版本条目与 tag 版本一致；同时约定 changelog 版本条目的统一格式，保证闸门可机器解析比对。
