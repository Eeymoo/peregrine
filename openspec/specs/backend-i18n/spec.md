# backend-i18n Specification

## Purpose

定义 Peregrine 后端（`src-tauri` 二进制层）的国际化能力：通过 `include_str!` 在编译期将前端共享的 6 语 locale JSON 嵌入二进制，运行时由 `serde_json` 反序列化为翻译表；提供统一的 `translate(locale, key)` 查表与回退入口；废除 `BackendLocale` 硬编码枚举；统一前后端 locale 检测的前缀映射语义；将 fallback 语言改为英文；并约束 `options.json` 语言选项标签本地化、6 门目标语言 locale 文件齐全与 key 集合一致。
